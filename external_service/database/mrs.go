...
func (c *Client) GetAllMRs() (mrs []models.MR, err error) {
	q := `SELECT id, url, author_id, is_closed, jira_id, jira_priority, gitlab_id FROM mrs`
	...
	var mr models.MR
	for rows.Next() {
		if err = rows.Scan(&mr.ID, &mr.URL, &mr.AuthorID, &mr.IsClosed, &mr.JiraID, &mr.JiraPriority, &mr.GitlabID); err != nil {
			err = ce.WrapWithLog(err, "get opened mrs")
			return
		}
		mrs = append(mrs, mr)
...
func (c *Client) CreateMR(mr models.MR) (models.MR, error) {
	q := `INSERT INTO mrs (url, author_id, gitlab_id) VALUES ($1,$2,$3) RETURNING id`
	err := c.db.QueryRow(q, mr.URL, mr.AuthorID, mr.GitlabID).Scan(&mr.ID)
	if err != nil {
...
func (c *Client) SaveMR(mr models.MR) (models.MR, error) {
	q := `UPDATE mrs SET is_closed=$2, jira_id=$3, jira_priority=$4, gitlab_id=$5 WHERE id=$1`
	_, err := c.db.Exec(q, mr.ID, mr.IsClosed, mr.JiraID, mr.JiraPriority, mr.GitlabID)
	if err != nil {
...
func (c *Client) GetOpenedMRs() (mrs []models.MR, err error) {
	q := `SELECT id, url, author_id, is_closed, jira_id, jira_priority, gitlab_id FROM mrs WHERE is_closed = FALSE`
	rows, err := c.db.Query(q)
	if err != nil {
		err = ce.WrapWithLog(err, "get opened mrs")
		return
...
func (c *Client) CloseMRs() (mrs []models.MR, err error) {
	...
	var mr models.MR
	for rows.Next() {
		if err = rows.Scan(&mr.ID, &mr.URL, &mr.AuthorID, &mr.IsClosed, &mr.GitlabID); err != nil {
...
func (c *Client) GetMrByID(id int) (mr models.MR, err error) {
	q := `SELECT id, url, author_id, jira_id, jira_priority, is_closed, gitlab_id FROM mrs WHERE id = $1`
	err = c.db.QueryRow(q, id).Scan(&mr.ID, &mr.URL, &mr.AuthorID, &mr.JiraID, &mr.JiraPriority, &mr.IsClosed, &mr.GitlabID)
	if err != nil {
